"""Profile integrated spectra memory across input and output precisions.
...
Usage:
    python profile_integrated_spectra_memory_scaling.py --basename test
"""
...
def _sample_integrated_spectra(
    method,
    inputs,
    out_dtype,
    nthreads,
    ...
    """Run integrated spectra repeats while continuously sampling RSS.
...
def plot_results(results, output_path):
    ...
    figure.suptitle("Integrated Spectra Memory Scaling")
    figure.tight_layout()
    figure.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(figure)


def profile_integrated_spectra_memory_scaling(
    basename,
    out_dir,
    nparticles,
    ngrid,
    ...
    """Run integrated spectra memory scaling benchmarks."""
    rng = np.random.default_rng(seed)

    output_dir = Path(out_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = output_dir / f"{basename}_integrated_spectra_memory_scaling.csv"
    plot_path = (
        output_dir / f"{basename}_integrated_spectra_memory_scaling.png"
    )

    results = []
    for particle_count in nparticles:
        print(
            "Profiling integrated spectra memory for "
        ...
        for method in methods:
            for input_name in PRECISIONS:
                for output_name, output_dtype in PRECISIONS.items():
                    result = _sample_integrated_spectra(
                        method,
...
if __name__ == "__main__":
    ...
    profile_integrated_spectra_memory_scaling(
