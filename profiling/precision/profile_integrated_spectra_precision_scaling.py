"""Profile integrated spectra scaling across input and output precisions.

This script benchmarks the integrated spectra extension for a synthetic 1D grid
...
Usage:
    python profile_integrated_spectra_precision_scaling.py --basename test
"""
...
def make_synthetic_inputs(nparticles, ngrid, nlam, dtype, rng):
    """Create contiguous synthetic inputs for integrated spectra."""
    ...
    return {
        ...
        "masses": np.array(masses, dtype=dtype, order="C", copy=True),
        "grid_dims": np.array([ngrid], dtype=np.int32),
    }


def benchmark_integrated_spectra(
    method,
    inputs,
    out_dtype,
    nthreads,
    ...
    """Time repeated integrated spectra calls for prepared inputs."""
...
def plot_results(results, output_path):
    ...
    figure.suptitle("Integrated Spectra Precision Scaling")
    figure.tight_layout()
    figure.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(figure)


def profile_integrated_spectra_precision_scaling(
    basename,
    out_dir,
    nparticles,
    ngrid,
    ...
    """Run the integrated spectra precision scaling benchmarks."""
    rng = np.random.default_rng(seed)

    output_dir = Path(out_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = (
        output_dir / f"{basename}_integrated_spectra_precision_scaling.csv"
    )
    plot_path = (
        output_dir / f"{basename}_integrated_spectra_precision_scaling.png"
    )

    results = []
    for particle_count in nparticles:
        print(
            "Profiling integrated spectra precision for "
        ...
        for method in methods:
            for input_name in PRECISIONS:
                for output_name, output_dtype in PRECISIONS.items():
                    benchmark_result = benchmark_integrated_spectra(
                        method,
...
if __name__ == "__main__":
    ...
    profile_integrated_spectra_precision_scaling(
