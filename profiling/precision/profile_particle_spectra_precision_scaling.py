"""Profile particle spectra scaling across input and output precisions.
...
Usage:
    python profile_particle_spectra_precision_scaling.py --basename test
"""
...
def benchmark_particle_spectra(method, inputs, out_dtype, nthreads, repeats):
    """Time repeated particle spectra calls for prepared inputs."""
...
def plot_results(results, output_path):
    ...
    figure.suptitle("Particle Spectra Precision Scaling")
    figure.tight_layout()
    figure.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(figure)


def profile_particle_spectra_precision_scaling(
    basename,
    out_dir,
    nparticles,
    ngrid,
    ...
    """Run the particle spectra precision scaling benchmarks."""
    rng = np.random.default_rng(seed)

    output_dir = Path(out_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = (
        output_dir / f"{basename}_particle_spectra_precision_scaling.csv"
    )
    plot_path = (
        output_dir / f"{basename}_particle_spectra_precision_scaling.png"
    )

    results = []
    for particle_count in nparticles:
        print(
            "Profiling particle spectra precision for "
        ...
        for method in methods:
            for input_name in PRECISIONS:
                for output_name, output_dtype in PRECISIONS.items():
                    benchmark_result = benchmark_particle_spectra(
                        method,
...
if __name__ == "__main__":
    ...
    profile_particle_spectra_precision_scaling(
