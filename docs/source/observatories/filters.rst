...
Filters can be used for producing photometry from ``Sed`` objects, as well as for creating monochromatic or RGB images. In practice they are most commonly attached to ``PhotometricInstrument`` or ``PhotometricImager`` objects.
