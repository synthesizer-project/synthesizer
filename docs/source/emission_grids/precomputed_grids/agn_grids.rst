...
The photonisation modelling parameters are included in the grid file attributes. Pre-computed grids are listed in the `Syndex catalogue <https://synthesizer-project.org/syndex>`_ and can be downloaded with ``synthesizer-download --dataset <name>``; AGN models without any photoionisation modelling are stored under `incident`, and for our default photoionisation assumptions under `photoionised`.
