...
- Performance: Computationally intensive operations are optimized by employing C extensions with OpenMP threading. Without this performance the aforementioned flexibility is moot, only by coupling flexibility with the performance to utilise it can we explore large parameter spaces in a reasonable time.
