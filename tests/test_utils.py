...
class TestEnsureCompatibles:
    ...
    def test_c_compatible_array_scalar(self):
        ...
        assert result == target_dtype.type(1.0), (
            f"Expected a single float value to be returned as {target_dtype} "
            f"but got {result}"
        )
    ...
    def test_c_compatible_array_list(self):
        ...
        assert result.dtype == target_dtype, (
            f"Expected a numpy array of type {target_dtype} but got "
            f"{result.dtype}"
        )
    ...
    def test_c_compatible_array_unyt32(self):
        ...
        assert isinstance(result, unyt.unyt_array), (
            f"Expected a unyt array to be returned but got {type(result)}"
        )
        assert result.dtype == target_dtype, (
            f"Expected a numpy array of type {target_dtype} but got "
            f"{result.dtype}"
