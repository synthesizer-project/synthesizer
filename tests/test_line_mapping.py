...
from synthesizer.instruments import Instrument, LineImager
...
class TestLineImager:
    ...
    def test_construction(self, line_ids):
        """A LineImager should construct with line_ids and resolution."""
    ...
    def test_factory_construction(self, line_ids):
        """Instrument should dispatch line mapping configurations."""
        inst = Instrument("test", line_ids=line_ids, resolution=1 * kpc)
        assert isinstance(inst, LineImager)
    ...
    def test_hdf5_roundtrip_with_scalar_snr(self, line_ids, tmp_path):
        """Dimensionless scalar SNRs should serialise without wrappers."""
        import h5py
        ...
        path = tmp_path / "line_imager_noise.hdf5"
        with h5py.File(path, "w") as hdf:
            inst.to_hdf5(hdf)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (1e30 * erg / s, "erg/s"),
            (1e-17 * erg / s / cm**2, "erg/s/cm**2"),
        ],
    )
    def test_noise_units_match_integrated_lines(
        self, line_ids, depth, expected_unit
    ):
        """Line-map noise uses integrated line units, not spectral density."""
